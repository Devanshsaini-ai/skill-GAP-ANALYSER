...
def analyze_skill_gap(resume_skills, job_title):
    ...
    if not client:
        ...
        return {"matching_skills": [], "missing_skills": [], "analysis": "", "match_percentage": 0.0}

    prompt = (
        f"Analyze the skill gap between these resume skills: {resume_skills} "
        f"and this target job title: {job_title}. "
        "Return a JSON object with three keys: "
        "'matching_skills' (list of skills from the resume relevant to the job), "
        "'missing_skills' (list of skills needed for the job but not in resume), "
        "'analysis' (a brief text analysis of the gap). "
        "Example: {\"matching_skills\": [\"Python\", \"SQL\"], \"missing_skills\": [\"AWS\", \"Docker\"], \"analysis\": \"The resume lacks cloud and containerization skills...\"}"
    )
    ...
    try:
        ...
        # Calculate Match Percentage based on Milestone formula
        matching_skills = result.get("matching_skills", [])
        missing_skills = result.get("missing_skills", [])
        
        total_job_skills = len(matching_skills) + len(missing_skills)
        if total_job_skills > 0:
            match_percent = (len(matching_skills) / total_job_skills) * 100
            result["match_percentage"] = round(match_percent, 2)
        else:
